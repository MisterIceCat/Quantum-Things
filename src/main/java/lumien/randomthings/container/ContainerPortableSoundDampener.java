...
public class ContainerPortableSoundDampener extends Container
{
	...
	public ContainerPortableSoundDampener(EntityPlayer player, World world, int x, int y, int z)
	{
		dampenerStack = player.inventory.getCurrentItem();
		inventory = ItemPortableSoundDampener.getInventory(dampenerStack);

		for (int i = 0; i < 9; i++)
		{
			this.addSlotToContainer(new SlotFiltered(inventory, i, 8 + i * 18, 18, is -> !is.isEmpty() && is.getItem() == ModItems.soundPattern && ItemSoundPattern.getSoundLocation(is) != null));
		}
